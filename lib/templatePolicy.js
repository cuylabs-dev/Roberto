...
export const NICHE_QUERIES = [
  ...
  { query: "spa masajes bienestar Lima", nicho: "Spa", template: "spas" },
  { query: "centro estetica facial corporal Lima", nicho: "Estética", template: "estetica" },
...
const BLOCKS_BY_TEMPLATE = {
  ...
  spas: ["reservas", "galeria"],
  estetica: ["reservas", "galeria"],
...
/** Detecta inmobiliaria aunque el nombre no diga "inmobiliaria" (ej. WIESTATE). */
export function looksLikeInmobiliaria(lead) {
  const hay = `${lead.name || ""} ${lead.maps_category || ""} ${lead.nicho || ""} ${lead.search_query || ""}`;
  return /inmobiliaria|bienes\s*ra[ií]ces|real\s*estate|realtor|inmueble|propiedades|corretaj|wiestate|\bestate\b|arriendo|agencia inmobili|agente inmobiliario|venta de inmuebles|alquiler de departamentos|housing/i.test(
    hay,
  );
}
...
const CATEGORY_RULES = [
  ...
  {
    re: /inmobiliaria|bienes raices|real estate|realtor|inmueble|propiedades|wiestate|corretaje|arriendo|agencia inmobiliaria/i,
    template: "inmobiliarias",
  },
  { re: /estetic|belleza|facial|corporal|laser|botox|cosmetolog|depilaci/i, template: "estetica" },
  { re: /spa|masaje|hammam|relax|wellness center/i, template: "spas" },
  { re: /barber|peluquer/i, template: "estetica" },
...
export function resolveTemplateFromLead(lead) {
  ...
  if (looksLikeInmobiliaria(lead)) return "inmobiliarias";
...
/** Farmacias / veterinarias fuera del pipeline outbound. */
export function isExcludedRubro(lead) {
  const hay = `${lead.name || ""} ${lead.maps_category || ""} ${lead.address || ""}`;
  return /farmacia|botica|farmac[eé]utic|pharma|veterinar|pet shop|mascotas|animal health/i.test(hay);
}
