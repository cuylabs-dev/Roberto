...
import { shortenBrandName, formatHeroHeadline } from "./displayText.js";
import { looksLikeInmobiliaria } from "./templatePolicy.js";
...
const NAME_BRAND_RULES = [
  ...
  { re: /wiestate|inmobiliar|propiedades|bienesraices|realtor/i, primary: "b45309", variant: "corp" },
  { re: /estetic|belleza|nova|derma|facial|laser|skin/i, primary: "db2777", variant: "care" },
  { re: /spa|masaje|hammam|relax/i, primary: "0d9488", variant: "care" },
...
export function localCopyFor(lead, template, variant) {
  ...
  const short = shortenBrandName(name, 36);
  ...
  const byTemplate = {
    ...
    clinicas: {
      headline: formatHeroHeadline(`Salud y cuidado en ${short}`, name, 52),
      subhead: `Citas, especialidades y atencion cercana en ${short}.`,
      eyebrow: "Clinica · Lima",
    },
    estetica: {
      headline: formatHeroHeadline(`Tu piel y figura en ${short}`, name, 52),
      subhead: `Tratamientos faciales y corporales con tecnologia y resultados visibles.`,
      eyebrow: "Estetica · Surco y Lima",
    },
    spas: {
      headline: formatHeroHeadline(`Momento spa en ${short}`, name, 52),
      subhead: `Masajes, rituales y bienestar para desconectar sin salir de la ciudad.`,
    ...
    inmobiliarias: {
      headline: formatHeroHeadline(`Tu proximo hogar con ${short}`, name, 52),
      subhead: `Compra, venta y alquiler de inmuebles en Lima con asesoria transparente en ${short}.`,
      eyebrow: "Inmobiliaria · Lima",
    },
    hoteles: {
      headline: formatHeroHeadline(`Hospedaje en ${short}`, name, 52),
      subhead: `Reserva directa, habitaciones comodas y mejor tarifa en ${short}.`,
      eyebrow: "Hotel · Lima",
    },
    corporativo: [
      {
        headline: `${short}: consultoria que ordena tu operacion`,
        subhead: `Diagnostico, plan de accion y ejecucion con foco en resultados para equipos en Lima.`,
        eyebrow: "Consultoria operativa",
      },
      {
        headline: `${short} — estrategia digital para PYMEs`,
        subhead: `Web, procesos y datos alineados a como vendes y atiendes hoy.`,
        eyebrow: "Transformacion digital",
      },
      {
        headline: `Talento y cultura con ${short}`,
        subhead: `Capacitacion, liderazgo y equipos de alto rendimiento sin discurso generico.`,
        eyebrow: "RRHH y cultura",
      },
      {
        headline: `${short}: finanzas y control para crecer`,
        subhead: `Ordenamos costos, flujo y tableros para decidir con numeros claros.`,
        eyebrow: "Consultoria financiera",
      },
      {
        headline: `Marketing y ventas con ${short}`,
        subhead: `Posicionamiento, embudo comercial y contenido que convierte en Peru.`,
        eyebrow: "Growth B2B",
      },
    ][nameSeed(name) % 5],
  };
  ...
  const pack =
    byTemplate[template] ||
    (looksLikeInmobiliaria(lead) ? byTemplate.inmobiliarias : null) ||
    byTemplate.corporativo;
  return {
...
const VISION_PROMPT = (lead, rubro) => `Analiza el logo o foto del negocio "${lead.name}" (rubro: ${rubro}) en Peru.
Extrae SOLO dos colores HEX reales de la marca (logo/fachada/local). No blanco puro ni gris neutro.
Responde UNICAMENTE este JSON sin otros campos:
{"primary":"#RRGGBB","secondary":"#RRGGBB"}`;

export async function analyzeBrandFromLogo(logoUrl, lead, rubro) {
  if (!logoUrl || !config.geminiApiKeys?.length) return null;
  ...
  try {
    ...
    return {
      ...
      brand_secondary: secondary || primary,
    };
  } catch (err) {
