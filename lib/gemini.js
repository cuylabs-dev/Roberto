...
import { inferRubroSpec } from "./rubrosCatalog.js";
import {
...
function mergeResult(base, visual, parsed, hint, lead) {
  ...
  const rubro = inferRubroSpec(lead);
  let blocks = Array.isArray(parsed?.blocks)
    ? parsed.blocks.map((b) => clampEnum(b, BLOCKS, null)).filter(Boolean)
    : [];
  if (!blocks.length && rubro.blocks?.length) {
    blocks = rubro.blocks.map((b) => clampEnum(b, BLOCKS, null)).filter(Boolean);
  }
...
export async function evaluateWithGemini(lead) {
  ...
  const rubro = inferRubroSpec(lead);
  if (!lead.template_hint && rubro.template) hint.template = rubro.template;
