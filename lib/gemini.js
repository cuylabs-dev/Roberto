...
${hint.template === "inmobiliarias" ? "INMOBILIARIA: compra/venta/alquiler, distritos de Lima, tipos de inmueble. PROHIBIDO consultoria, diagnostico B2B, holding o copy corporativo generico." : ""}
${hint.template === "corporativo" ? "CONSULTORA/EMPRESA LOCAL: servicios concretos del negocio. PROHIBIDO texto de multinacional o grupo regional." : ""}
${hint.template === "estetica" ? "CENTRO DE ESTETICA: tratamientos faciales/corporales reales (laser, limpieza, radiofrecuencia). PROHIBIDO copy de clinica general o recuperacion medica." : ""}
${hint.template === "spas" ? "SPA: rituales, masajes, bienestar. PROHIBIDO copy hospitalario." : ""}
...
SOLO JSON:
{
  "headline": "max 48 chars, nombre corto del negocio (SIN ' - Sede X'), servicio estrella",
...
function mergeResult(base, visual, parsed, hint, lead) {
  ...
  const headline = formatHeroHeadline(
    clip(parsed?.headline || base.headline, 70),
    lead.name,
    52,
  );
  const subhead =
    formatHeroSubhead(clip(parsed?.subhead || base.subhead, 160), 150) ||
    clip(base.subhead, 150);
...
export async function evaluateWithGemini(lead) {
  const hint = inferFromNiche({
    ...
    name: lead.name,
    template_hint: lead.template_hint,
