...
import { formatHeroHeadline, formatHeroSubhead } from "./displayText.js";
...
import { buildSectionsPrompt } from "./copyPrompts.js";
...
const FONT_PAIRS = {
  ...
  inmobiliarias: { heading: "Playfair Display", body: "Inter" },
  estetica: { heading: "Poppins", body: "Inter" },
  spas: { heading: "Poppins", body: "Inter" },
};
...
function defaultSectionsCopy(template, lead) {
  ...
  const byTemplate = {
    ...
    estetica: {
      tratamientos: {
        title: "Tratamientos",
        subtitle: `Protocolos en ${name}`,
        items: ["Limpieza facial profunda", "Radiofrecuencia facial", "Depilación láser", "Peeling químico"],
      },
      servicios: {
        title: "Servicios estrella",
        items: ["Evaluación gratuita", "Paquetes faciales", "Corporales", "Domicilio Surco"],
      },
      stats: { items: ["+8 años", "4.9 en Google", "Tecnología FDA", "Equipo cosmetólogas"] },
      testimonios: [
        { quote: `Mi piel cambió desde la primera sesión en ${name}.`, author: "Valeria T.", role: "Facial" },
        { quote: "Resultados visibles sin dolor.", author: "Camila R.", role: "Láser" },
      ],
    },
    spas: {
      rituales: {
        title: "Rituales",
        subtitle: `Desconecta en ${name}`,
        items: ["Masaje relajante 60 min", "Facial hidratante", "Circuito aromaterapia", "Piedras calientes"],
      },
      servicios: {
        title: "Experiencias",
        items: ["Day spa parejas", "Masaje deportivo", "Exfoliación corporal"],
      },
      stats: { items: ["Ambiente premium", "Terapeutas certificadas", "Productos veganos", "Reserva WhatsApp"] },
      testimonios: [
        { quote: `El mejor masaje que he tenido en Lima — ${name}.`, author: "Andrea L.", role: "Cliente frecuente" },
      ],
    },
    clinicas: {
    ...
    inmobiliarias: {
      hero: {
        eyebrow: `Inmobiliaria en Lima`,
        tagline: `${name} — asesoría para comprar, vender o alquilar con confianza`,
      },
      propiedades: {
        title: "Propiedades destacadas",
        subtitle: `Oportunidades activas con ${name}`,
        items: [
          { titulo: "Departamento 3 dorm. Miraflores", detalle: "Vista parque · 92 m² · desde S/ 485,000", zona: "Miraflores" },
          { titulo: "Casa en condominio La Molina", detalle: "4 dorm. · cochera · S/ 720,000", zona: "La Molina" },
          { titulo: "Oficina amoblada San Isidro", detalle: "45 m² · mantenimiento incluido · alquiler", zona: "San Isidro" },
        ],
      },
      zonas: {
        title: "Zonas donde operamos",
        items: ["Miraflores y Barranco", "San Isidro y Magdalena", "Surco y La Molina", "San Borja y Lince"],
      },
      servicios: {
        title: "Servicios",
        subtitle: `Todo el ciclo inmobiliario con ${name}`,
        items: ["Venta de inmuebles", "Alquiler residencial y corporativo", "Tasación y estudio de mercado", "Asesoría legal y SUNARP"],
      },
      proceso: {
        title: "Cómo trabajamos",
        items: ["Diagnóstico de necesidad", "Selección de opciones reales", "Visitas y negociación", "Cierre seguro"],
      },
      stats: { items: ["+80 propiedades gestionadas", "10 años en el mercado limeño", "Asesores locales certificados", "4.8★ en Google"] },
      quienes_somos: {
        title: `Sobre ${name}`,
        subtitle: "Inmobiliaria boutique en Lima",
        body: `${name} acompaña a familias y empresas en la compra, venta y alquiler de inmuebles, con foco en transparencia y tiempos de respuesta cortos.`,
      },
      testimonios: [
        { quote: `${name} nos ayudó a cerrar el depto en Surco en menos de 45 días.`, author: "Carlos M.", role: "Comprador" },
        { quote: "Tasación clara y sin sorpresas al vender mi local.", author: "Lucía P.", role: "Propietaria" },
        { quote: "Encontraron inquilino corporativo para nuestra oficina.", author: "Gerente", role: "Alquiler B2B" },
        { quote: "Trato humano, no plantillas.", author: "Familia R.", role: "Miraflores" },
      ],
    },
    corporativo: {
      metricas: {
...
export async function generateSectionsCopy(lead, template, sections, brandPrimary) {
  try {
    const { parsed, llm_meta } = await llmGenerateValidatedJson({
      task: "sections",
      buildPrompt: () => buildSectionsPrompt(lead, template, sections, brandPrimary),
      validate: (p) => p?.sectionsCopy && !sectionsCopyIsGeneric(p.sectionsCopy),
      maxSteps: 7,
    });
    if (parsed?.sectionsCopy) {
      if (llm_meta?.provider) {
        console.log(`   Kit secciones OK: ${llm_meta.provider}/${llm_meta.model}`);
      }
      return parsed;
    }
    console.log(`   Kit secciones: LLM devolvio copy generico — usando defaults locales`);
  } catch (err) {
...
export function buildBrandKit(lead, evalResult, social = {}) {
  ...
  const kit = {
    ...
    tone,
    copy: {
      head: formatHeroHeadline(evalResult.headline, lead.name, 52),
      sub: formatHeroSubhead(evalResult.subhead, 150) || evalResult.subhead,
...
export async function finalizeBrandKit(kit, lead = null) {
  ...
  if (llmExtra?.sectionsCopy) {
    ...
    const heroBlock = llmExtra.sectionsCopy.hero;
    if (heroBlock && typeof heroBlock === "object") {
      if (heroBlock.eyebrow) kit.copy.eyebrow = heroBlock.eyebrow;
      if (heroBlock.tagline) kit.copy.sub = heroBlock.tagline;
    }
  }
